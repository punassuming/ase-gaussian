...
import exceptions, glob, os, pickle, string, time, uuid
...
class Jacapo:
    ...
    def get(self, kw):
        '''get a parameter without reading the ncfile'''
        return self.pars[kw]        
    ...
    def write_input(self):
        ...
        if hasattr(self,'tags'):
            ncf.tags = self.get_tags()

        # optional long text description
        if hasattr(self,'description'):
            ncf.description = self.get_description()
    ...
    def get_tags(self):
        nc = netCDF(self.nc)
        tags = getattr(nc,'tags',None)
        nc.close()
        return tags

    def set_tags(self,listoftags,replace=True):     
        if replace:
    ...
    def get_description(self):
        nc = netCDF(self.nc)
        description = getattr(nc,'description',None)
        nc.close()
        return description

    def set_description(self,description,replace=True):
        if replace:
...
#                  self.get_magnetic_moment()) #can trigger a calculation
...
##                    #self.restart()
    ...
    def get_date_created(self):
        nc = netCDF(self.nc, 'r')
        date_created = getattr(nc,'date_created',None)
        nc.close()
        return date_created

    def get_absolute_path(self):
        return os.path.abspath(self.get_nc())
    ...
    def get_dacapo_version(self):
        ncf = netCDF(self.get_nc(), 'r')
        version = getattr(ncf,'version',None)
        ncf.close()
        return version

    def get_jacapo_version(self):
        ncf = netCDF(self.get_nc(), 'r')
        jacapo_version = getattr(ncf,'Jacapo_version',None)
        ncf.close()
        return jacapo_version 
    ...
    def calculate(self):
        ...
        if self.stay_alive:
            ...
            if status is 0: #that means it ended fine!
                ...
                self.set_status('finished')
                try:
                    insert_database_entry(self)
                except:
                    log.debug('Database entry failed')
            else:
    ...
    def write_nc(self, nc=None, atoms=None):
        ...
        if atoms is not None: #there may still be no atoms attached
            ...
            for sym in atoms.get_chemical_symbols():
                ...
                # get md5 of pspotfile
                if os.path.exists(ppath):
                    fullpath = ppath
                elif os.path.exists(os.path.join(os.environ['DACAPOPATH'],ppath)):
                    fullpath = os.path.join(os.environ['DACAPOPATH'],ppath)
                else:
                    raise Exception,'%s not found' % ppath
